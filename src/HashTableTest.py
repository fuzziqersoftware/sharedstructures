...
def run_conditional_writes_test(allocator_type):
    ...
    conditional_insert_both(
        expected, table, b"key2", b"value1", b"key1", b"value1", False
    )
    verify_state(expected, table)
    conditional_insert_both(
        expected, table, b"key2", b"value2", b"key1", b"value1", True
    )
    verify_state(expected, table)

    # Check that missing conditions work
    conditional_missing_insert_both(expected, table, b"key2", b"key3", b"value3", False)
    verify_state(expected, table)
    conditional_missing_insert_both(expected, table, b"key3", b"key3", b"value3", True)
    verify_state(expected, table)
...
def run_incr_test(allocator_type):
    ...
    table = sharedstructures.HashTable(
        POOL_NAME_PREFIX + allocator_type, allocator_type, 0, 6
    )
    expected = {}
