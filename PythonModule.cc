...
// PrefixTree and PrefixTreeIterator definitions
...
static pair<const char*, size_t> sharedstructures_PrefixTree_internal_get_key(
    PyObject* key) {
...
static PyObject* sharedstructures_PrefixTree_New(PyTypeObject* type,
  ...
  try {
    ...
    new (&self->table) sharedstructures::PrefixTree(pool, 0);
  } catch (const exception& e) {
...
static int sharedstructures_PrefixTree_In(PyObject* py_self, PyObject* key) {
  ...
  auto k = sharedstructures_PrefixTree_internal_get_key(key);
  if (!k.first) {
    return -1;
  }
